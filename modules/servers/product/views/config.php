<!-- Different -->

<?php

/* Language */
/* The language your peak here, will be set as your panel default language : */
/* Turkish ==>> "Turkish", Farsi   ==>> "Farsi", English ==>> "English", Russian ==>> "Russian" , Deutsch ==>> "Deutsch", French ==>> "French", Italian ==>> "Italian", Brizilian ==>> "Brizilian",*/
$CloudDefaulLanguage = "English";




/* Currency PlaceHolder */
/* Between Prefix and Suffix, you should pick one that works for you  */
/* Pick from this list [ "suffix" , "prefix"] */
$currencySymbolPlace = "prefix";




/* Backlink */
/* Here you can write the link of (back to arrow) on top of the panel */
/* if you change the (admin address) you shoul write the new one here */
$DefaultClientBackLink = '/clientarea.php?action=productdetails';
$DefaultAdminBackLink = '/admin/clientsservices.php';

?>


